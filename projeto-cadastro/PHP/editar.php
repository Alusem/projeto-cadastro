...
if (isset($erros) && count($erros) > 0) {
    ...
    header("Location: ../View/editar-cliente.php?idClientes=". $_POST['idClientes']);
    ...
    $email = addslashes($_POST['email']);
	 ...
	 try {
        ...
        $stmt = $pdo->prepare('UPDATE clientes SET nomeClientes = :nome, cpfClientes = :cpf, apelidoClientes = :apelido, telefoneClientes = :telefone, emailClientes = :email WHERE idClientes=:id');
  ...
  if (! $retorno){
      $erros[] = utf8_encode('Não foi possivel atualizar o cadastro, Tente Novamente');
  }else {
       ?> <script>alert("Cliente editado com sucesso!");</script> <?php
